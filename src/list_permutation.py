...
# to use different for loops to append values one by one. These examples might help.
#
...
# equal to every element one at a time. It will then build up a list by evaluating the
# expression-involving-loop-variable for each one. This eliminates the need to use lambda
# forms and generally produces a much more readable code than using map() and a more compact code than using a for loop.
#
...
# [[1, 4], [1, 5], [1, 6], [2, 4], [2, 5], [2, 6], [3, 4], [3, 5], [3, 6]]
#
# The final form of list comprehension involves creating a list and filtering it similar
# to using the filter() method. The filtering form of list comprehension takes the following form:
#
...
z = int(input())
n = int(input())
print([[x0, y0, z0] for x0 in range(0, x + 1) for y0 in range(0, y + 1) for z0 in range(0, z + 1) if x0 + y0 + z0 != n])
